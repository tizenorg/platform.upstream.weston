...
wayland_output_get_shm_buffer(struct wayland_output *output)
{
	...
	sb = zalloc(sizeof *sb);
	if (sb == NULL) {
		weston_log("could not zalloc %u memory for sb: %m\n", sizeof *sb);
		close(fd);
		free(data);
		return NULL;
	}
...
input_capabilities_updated(struct wayland_input *input)
{
	if ((input->caps & WL_SEAT_CAPABILITY_POINTER) &&
		...
		input->parent.pointer = wl_seat_get_pointer(
				input->parent.seat);
...
{
	struct wayland_parent_output *output = data;
	if (output->c->seatfilter &&
		(!output->seatname || !output->c || !output->c->seatname ||
...
wayland_compositor_register_output(struct wayland_compositor *c, uint32_t id)
{
	...
	wl_list_insert(c->parent.output_list.prev, &output->link);
}
	     ...
	     struct weston_config *config)
{
	...
	const struct weston_option wayland_options[] = {
		...
		{ WESTON_OPTION_BOOLEAN, "seatfilter", 0, &seatfilter },
